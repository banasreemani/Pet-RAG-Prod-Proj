...
#from implementation.mypetanswer import answer_question, fetch_context
from app.mypetanswer import answer_question, fetch_context
...
#MODEL = "gpt-4.1-nano"
#MODEL = "ollama/llama3.2" #ChatOllama(model="llama3.2") // 19-06-26
...
def evaluate_answer(test: TestQuestion) -> tuple[AnswerEval, str, list]:
    ...
    generated_answer, retrieved_docs, trace_id = answer_question(test.question)
...
def evaluate_all_answers():
    ...
    total_tests = len(tests)
    #for index, test in enumerate(tests):
    for index, test in enumerate(tests[:5]):    #only select 5, With 50 tests, answer evaluation can take a very long time.
...
def run_cli_evaluation(test_number: int):
    ...
    tests = load_tests("testsnew.jsonl") #pet care
